...
```
5.1 ✅ ผ่านแล้ว → 5.2 → 5.3 → 5.8ข (ข้อมูลร้าน) → 5.4 → 5.5 → 5.6 → 5.7 → 5.8ก (ผู้ใช้)
```
...
GET /reports/inventory-value     → 2 กล่องล่าง  🔒 owner เท่านั้น (D20)
```
...
| ทำอะไร | ต้องเห็นอะไร |
|---|---|
| owner เปิด `/dashboard` | กล่อง 4 ระดับมีทั้งจำนวนและมูลค่า · 3 อันดับแรกแสดงตรงหน้า · 2 กล่องล่างมี % |
| pharmacist เปิด `/dashboard` | ไม่มี 2 กล่องล่าง · ไม่เหลือช่องว่าง |
