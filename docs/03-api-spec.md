...
### 9.9 ผังร้าน (U-8 — ตัวเลือกเสริม ไม่ใช่ระบบหลัก)

> ผังเป็น **อีกวิธีกรอก** ที่ผู้ใช้จะใช้หรือไม่ใช้ก็ได้
> ระบบหลัก สแกน → เข้าคลัง → ขายยา (พิมพ์/ยิงบาร์โค้ด) ไม่ได้พึ่งอะไรจากส่วนนี้เลย
> ลบทั้ง 4 ตารางและ endpoint ทั้งหมดทิ้ง ระบบหลักยังทำงานครบ

| Method | Path | สิทธิ์ | คำอธิบาย |
|---|---|---|---|
| GET | `/api/v1/store-map` | **ทุก Role** | ผัง + ของในห้อง + จุดทั้งหมด ในคำตอบเดียว |
| PUT | `/api/v1/store-map` | owner | สร้างผัง หรือแก้ชื่อ/ขนาดห้อง |
| POST | `/api/v1/store-map/shapes` | owner | เพิ่มของในห้อง (ชั้นวาง เคาน์เตอร์ เสา ประตู) |
| PATCH | `/api/v1/store-map/shapes/{id}` | owner | ย้าย/แก้ |
| DELETE | `/api/v1/store-map/shapes/{id}` | owner | |
| POST | `/api/v1/store-map/points` | owner | เพิ่มจุด mark |
| PATCH | `/api/v1/store-map/points/{id}` | owner | ย้าย/เปลี่ยนชื่อ |
| DELETE | `/api/v1/store-map/points/{id}` | owner | ลบจุด (ลิงก์ยาหายตามด้วย cascade) |

**อ่านได้ทุก Role เพราะพนักงานขายต้องหาว่ายาอยู่ตรงไหน · แก้ได้เฉพาะ owner เหมือนข้อมูลร้าน**

- `GET` **ไม่เคยตอบ 404** — ร้านที่ยังไม่ได้วาดผังจะได้ `{"map": null, "shapes": [], "points": []}`
  หน้าจอจึงบอกเป็นข้อความว่ายังไม่มีผัง แทนที่จะวาดห้องเปล่า
- **ไม่มีฟิลด์เกี่ยวกับเงินในคำตอบใดเลย** (D32/D33) ผังตอบว่า *อยู่ตรงไหน* ไม่ตอบว่า *ราคาเท่าไร*
  มี check ถาวรกันไว้ที่ `store-map-access.check.mjs` ซึ่งอ่านทั้ง `abilities.ts` และ router จริง
- `medicine_count` ของแต่ละจุด **นับด้วยฐานข้อมูล** ไม่ได้ส่งลิงก์ทั้งหมดมาให้เบราว์เซอร์นับเอง
- เพดาน **1000 จุด / 200 ชิ้นต่อผัง** บังคับที่ service ไม่ใช่ที่ constraint
  (constraint ที่ต้องนับแถวทุกครั้งที่ insert จะช้าลงเรื่อย ๆ ตามจำนวนจุด)
- ของหรือจุดที่วางนอกกรอบผัง → `400` · รหัสจุดซ้ำในผังเดียวกัน (ไม่สนตัวพิมพ์เล็ก/ใหญ่) → `409`
- ย่อผังให้เล็กกว่าของที่วางไว้แล้ว → `400` แทนที่จะปล่อยให้ชั้นวางไปอยู่นอกกำแพงเงียบ ๆ

### 9.10 ถ่ายรูปใบส่งของ (Phase 6 งาน 6.2)

| Method | Path | สิทธิ์ | คำอธิบาย |
|---|---|---|---|
| POST | `/api/v1/invoice-scans` | owner · pharmacist | เริ่มใบใหม่ (สถานะ `uploaded`) — หน้าจอเรียกตอนรูปแรก ไม่ใช่ตอนเปิดหน้า |
| GET | `/api/v1/invoice-scans` | owner · pharmacist | 50 ใบล่าสุด พร้อม `page_count` · **ไม่มีลิงก์รูป** |
| GET | `/api/v1/invoice-scans/{id}` | owner · pharmacist | ใบ + ทุกหน้า · แต่ละหน้ามี `image_url` ของ **ฉบับหลักฐาน (original)** เป็น signed URL อายุ `SIGNED_URL_TTL_SECONDS` สร้างใหม่ทุกครั้งที่อ่าน |
| POST | `/api/v1/invoice-scans/{id}/pages` | owner · pharmacist | ส่งรูป 1 หน้า **เป็น body ตรง ๆ** (ไม่ใช่ multipart) · ได้ `page_no` ถัดไป |
| DELETE | `/api/v1/invoice-scans/{id}/pages/{page_no}` | owner · pharmacist | ลบหน้า ทั้ง 2 แถวและ 2 ไฟล์ |

- รูป 1 หน้า → **2 ไฟล์ + 2 แถว** (`original` · `for_ai`) ตาม D68 · ทุกแถวบันทึกของที่มาถึงจริงตาม migration 012
- ด่านขนาด `MAX_UPLOAD_BYTES` (30 MB) อ่าน body เป็นก้อน **ตัดทันทีที่เกิน** ไม่เชื่อ Content-Length อย่างเดียว → `413` (D72)
- ชนิดไฟล์ตัดสินจากไบต์ ไม่ใช่ Content-Type (D69): HEIC · PDF · อื่น ๆ → `415` พร้อมข้อความที่บอกทางออก · ไฟล์เสีย/ไม่ครบ → `422` · body ว่าง → `400`
- ใบที่สถานะไม่ใช่ `uploaded` → เพิ่ม/ลบหน้าไม่ได้ `409` (รูปเป็นหลักฐานของสิ่งที่อ่านไปแล้ว)
- ลำดับงานต่อหน้า: ย่อ → อัปโหลด 2 ไฟล์ชื่อสุ่ม → transaction เดียว (ล็อกใบ · เลขหน้าถัดไป · insert 2 แถว · audit) → ถ้า transaction พัง ลบ 2 ไฟล์ทิ้ง · ไม่มีการเรียกเครือข่ายค้างใน transaction
- ลบหน้า: ลบแถวก่อน แล้วค่อยลบไฟล์ — ไฟล์ค้างคือแค่พื้นที่เปลือง แต่แถวที่ชี้ไปหาไฟล์ที่หายคือรูในหลักฐาน
- 🔴 **`for_ai` ไม่มีลิงก์ในคำตอบใดเลย** · AI อ่านรูปผ่าน `paths_for_ai()` ใน service เท่านั้น ซึ่งเลือก `kind = 'for_ai'` (D68)
- 🔴 **เว็บไม่เรียก Storage เองและไม่ประกอบที่อยู่ Storage เอง** — check ถาวรใน `source-rules.check.mjs`
- service_role key อยู่ที่ backend เท่านั้น ส่งใน header `apikey` (ทดสอบกับของจริงแล้ว: Bearer อย่างเดียวถูกปฏิเสธ — D67)
...
| — | 25 ก.ย. 2569 | โมเดลสำหรับสร้างท่อ 6.2–6.3: `gemini-3.5-flash-lite` | เลือกจากตัวที่**เรียกได้จริง** (ตระกูล 2.5 ปิดรับผู้ใช้ใหม่ · 3.7/3.8-flash ติด 503) · ราคาที่ Chat A หามา 0.30 / 2.50 ดอลลาร์ต่อล้าน token (เข้า/ออก) · ไม่คิดก่อนตอบ · เรียกจริง 3 ครั้ง 790 · 752 · 1,003 ms · token 8 เข้า · 1 ออก · คิด – · 🔴 **ไม่ใช่การเลือกถาวร** — 6.7 เทียบ 3.5-flash-lite · 3.6-flash · 3.1-flash-lite บนใบชุดเดียวกันที่มีใบเขียนมือ (D65) และเพิ่ม 3.7/3.8 ถ้าหายจาก 503 · ใช้ตัดสินว่าความแม่นที่เพิ่มขึ้นคุ้มกับเงินที่จ่ายเพิ่มหรือไม่ |
| — | 25 ก.ย. 2569 | token การคิด เก็บแยกคอลัมน์ | 🔴 **6.3 migration เพิ่ม `thinking_tokens int null` ใน `invoice_scans` ห้ามรวมกับ `tokens_out`** · เหตุผล: (1) คนละความหมาย — tokens_out คือของที่เราได้ใช้ thinking คือของที่จ่ายแล้วไม่ได้เห็น · (2) ต้องเทียบข้ามโมเดลได้ว่าตัวไหนคิดเกินจำเป็น · (3) อัตราคิดเงินอาจเปลี่ยนแยกกันในอนาคต · `null` = ผู้ให้บริการไม่ได้รายงาน · `0` = ไม่ได้คิด · ⚠️ **สมมติฐาน (ยังไม่ยืนยัน): token การคิดคิดเงินอัตราเดียวกับขาออก** — หน้าราคาไม่ได้ยืนยันตรง ๆ · ยืนยันจากหน้าการใช้งานของบัญชีหลังสแกนใบจริงประมาณ 10 ใบ · ที่มา: เรียกจริงพบว่า 3.5-flash ใช้ 106 token คิดเพื่อตอบคำเดียว |
| D66 | 25 ก.ย. 2569 | สคริปต์ที่แก้ซอร์สชั่วคราว ต้องคืนค่าใน finally | 🔴 **สคริปต์ที่แก้ซอร์สชั่วคราวเพื่อพิสูจน์ว่าเทสต์ FAIL ได้ ต้องคืนค่าใน `finally` เสมอ และต้องตรวจสถานะไฟล์ก่อน commit ทุกครั้ง** · ที่มา: สคริปต์ใส่บั๊กพังกลางคัน (หา python ไม่เจอ) หลังจากแก้ `gemini.py` ให้คีย์อยู่ใน URL ไปแล้ว — ไฟล์ค้างอยู่ในสภาพนั้น เจอและคืนจากสำรองก่อน commit · **เครื่องมือที่ใช้พิสูจน์ความปลอดภัย กลายเป็นตัวสร้างช่องโหว่เสียเอง** · วิธีทำ: สำรองก่อนแตะ · ใส่บั๊กใน `try` · คืนค่าใน `finally` · ตรวจว่าไฟล์ตรงกับสำรองทุกไบต์ · `git diff` ก่อน commit |
| D67 | 25 ก.ย. 2569 | ชื่อของภายนอก ต้องถามของจริงก่อน — และต้องเรียกจริง ไม่ใช่แค่ดูรายชื่อ | 🔴 **ก่อนล็อกชื่อของภายนอก (โมเดล · endpoint · รุ่น · เวอร์ชัน) ให้ถามของจริงก่อน ห้ามเขียนจากความจำ** · ที่มา: Chat A เดา `gemini-2.5-flash` · Claude Code เดา `gemini-2.0-flash` — ตัวหลังไม่มีอยู่จริง ทั้งคู่เดา ถูกหนึ่ง ผิดหนึ่ง · เทสต์ที่ไม่แตะเครือข่ายจับเรื่องนี้ไม่ได้เลย · 🔴 **ขยายความ (พบตอนรันจริง): อยู่ในรายชื่อ ≠ เรียกได้** — `gemini-2.5-flash` อยู่ในรายชื่อ และ metadata ของตัวมันเองบอกว่ารองรับ `generateContent` แต่เรียกจริงได้ **404 “no longer available to new users”** ทั้งตระกูล 2.5 (flash · flash-lite · pro) · ดังนั้น “ถามของจริง” ต้องหมายถึง**เรียกจริงหนึ่งครั้ง** ไม่ใช่แค่ดูว่ามีชื่ออยู่ |
| D68 | 25 ก.ย. 2569 | รูปหลักฐาน กับ รูปที่ส่งให้ AI เป็นคนละไฟล์ — และ “original” คือฉบับที่ใช้เป็นหลักฐาน | หนึ่งแถวต่อหนึ่งไฟล์ แยกด้วย `kind` (migration 011) · **เฉพาะ `for_ai` เท่านั้นที่ถูกส่งให้ AI** · **แก้ไข (migration 012):** `original` = ไฟล์ที่ย่อแล้วเก็บเป็นหลักฐาน **ไม่ใช่ไบต์ที่มือถือส่งมา** · ด้านยาว ≤ `ORIGINAL_MAX_EDGE_PX` (3000) q`ORIGINAL_JPEG_QUALITY` (88) · for_ai ≤ `AI_IMAGE_MAX_EDGE_PX` (1600) q`AI_IMAGE_JPEG_QUALITY` (80) · **เก็บตามเดิม ไม่เข้ารหัสซ้ำ เมื่อครบทุกข้อ:** เป็น JPEG · ด้านยาวไม่เกินเพดาน · ขนาดไม่เกินเพดานของ bucket · ไม่มี EXIF สั่งให้หมุน — ไม่ครบให้หมุนตาม EXIF แล้วเข้ารหัสเป็น JPEG · ทุกแถวบันทึกของที่มาถึงจริง (`source_content_type` · `source_bytes` · `source_width_px/height_px` · `source_sha256`) และ `reencoded` · 🔴 **ฐานข้อมูลบังคับ: `reencoded = false` ได้เฉพาะเมื่อขนาดไบต์ กว้าง สูง ตรงกับ source ทุกตัว** · เหตุผล: กฎข้อ 30 ต้องการเอกสารที่อ่านออกและเนื้อหาไม่ถูกแก้ ไม่ใช่ความละเอียดสูงสุด · 100 ใบ × 2 หน้า × 10 MB = 2 GB/เดือน ไม่มีวันลด · เราไม่เรียกของที่แก้แล้วว่าต้นฉบับโดยไม่บอก · ลายนิ้วมือคือสิ่งเดียวที่ตอบได้ว่ารูปนี้มาจากไฟล์ไหนถ้ามีข้อพิพาทกับบริษัทยา |
| D69 | 25 ก.ย. 2569 | ชนิดไฟล์ตัดสินจากไบต์จริง ไม่เชื่อป้ายของเบราว์เซอร์ | ผลทดสอบ iPhone จริง: ทั้ง “ถ่ายรูป” และ “เลือกจากคลังรูป” ได้ `image/jpeg` ไบต์จริง JPEG (6.9 MB · 18.7 MB) — iOS แปลงให้เอง **ไม่ต้องแปลง HEIC · ไม่ลง pillow-heif** · แต่ตัวอ่านชนิดจากไบต์ยังคงไว้ทั้งเว็บและ backend เพราะ Android อาจต่าง และ iOS รุ่นหน้าอาจเปลี่ยน · เจอ HEIC → 415 พร้อมทางออก (“ลองกดถ่ายรูปแทนการเลือกจากคลังรูป”) และลง log · ถ้าเจอบ่อยค่อยลง pillow-heif · PDF ถอดออกใน 6.2 (ย่อไม่ได้ และยังไม่ได้นิยามว่า for_ai ของ PDF คืออะไร) → 415 · bucket ยังรับ PDF ไว้ |
| D72 | 25 ก.ย. 2569 | ตัวเลขเพดาน ต้องบอกว่าวัดหรือเดา — และต้องรู้ว่าของไหลผ่านชั้นไหน | 🔴 **ตัวเลขเพดานหรือค่าคงที่ที่ Chat A ให้มา ต้องระบุเสมอว่า “วัดมาแล้ว” หรือ “เดา”** · ถ้าเดา ห้ามใช้เป็นด่านจริงจนกว่าจะวัด · **ก่อนตั้งด่าน ต้องตอบให้ได้ว่าของที่จะกันไหลผ่านชั้นไหนบ้าง** · ที่มา: bucket 10 MB (เดา) ปฏิเสธรูป iPhone 18.7 MB → สั่งขยายเป็น 30 MB (เดาอีกครั้ง) ทั้งที่ไฟล์ดิบไม่เคยขึ้น bucket — มันจบที่ backend · วัดจริงแล้ว: รูป 48 MP noise ล้วน 57.2 MB ย่อเหลือ 3.47 MB · เพดานสูงสุดที่เป็นไปได้ 3000×3000 = 7.5 MB · **ผล: bucket คง 10 MB · ด่านจริงคือ `MAX_UPLOAD_BYTES` = 30 MB ที่ backend อ่านเป็นก้อน ตัดทันทีที่เกิน ตอบ 413** |
| — | 25 ก.ย. 2569 | เอกสารที่ Chat A อ้าง ต้องอยู่ใน repo | 🔴 **Chat A อ้างเอกสารได้เฉพาะไฟล์ที่อยู่ใน repo** · ถ้าเนื้อหาอยู่ใน Project บน claude.ai ต้องวางเนื้อหามาในคำสั่ง ห้ามอ้างชื่อไฟล์ลอย ๆ · ถ้าเจอคำสั่งที่อ้างไฟล์ที่ไม่มีใน repo ให้**หยุดและขอเนื้อหา** ห้ามเดาจากชื่อไฟล์ · ที่มา: งาน 6.1 อ้าง `claude/docs-06-phase6-design.md` และ `…-findings.md` ซึ่งอยู่ใน Project ไม่ใช่ใน repo — migration 010 ต้องหยุดรอ 1 รอบ |
| D59 | 25 ก.ย. 2569 | บรรทัดที่มีจำนวนแต่ไม่มีราคา = ของแถม | **บรรทัดที่มีจำนวนแต่ไม่มีราคา คือของแถม ไม่ใช่ข้อมูลขาด** · ของแถมเข้าสต็อกจริง · ที่มา: ใบกำกับภาษีจริง (แบบ A) มี 9 รายการ เป็นของแถม 5 รายการ — มีจำนวน ไม่มีราคา ไม่มียอดเงิน · สินค้าตัวเดียวกัน LOT เดียวกัน ปรากฏได้ 3 บรรทัด: ซื้อ + แถม + แถมพิเศษ |
| D60 | 25 ก.ย. 2569 | ต้นทุน 2 ค่า | เก็บ `paid_unit_cost` (ราคาที่พิมพ์บนใบ) และ `effective_unit_cost` (**เงินที่จ่ายจริงทั้งก้อน ÷ ของที่ได้รับจริงทั้งหมด รวมของแถม**) · ค่าตั้งต้นที่ใช้คิดกำไรและมูลค่าสต็อก = **effective** · ปุ่มสลับดู = Phase 8 · 🔴 **สองคอลัมน์นี้ทำตอนงาน 6.6 ไม่ใช่ 6.1** (ต้องแตะ `medicine_lots` ซึ่งห้ามแตะใน 6.1) |
| D61 | 25 ก.ย. 2569 | ตรวจยอดตามโครงสร้างจริงของใบ | **Backend ตรวจยอดตามโครงสร้างจริงของใบ ไม่ใช่สูตรที่คิดเอง** · เหตุผล: เตือนผิดบ่อย ๆ คนจะเลิกอ่านคำเตือน แล้วระบบเตือนตายทั้งระบบ · โครงสร้างของใบจริง (แบบ A): ราคาที่พิมพ์**รวม VAT แล้ว** · ยอดรวม − ส่วนลด = **ยอดสุทธิ** · ยอดสุทธิ ÷ 1.07 = **ฐานก่อน VAT** · ฐาน + VAT = ยอดสุทธิ · ช่อง “ยอดเงินหลังหักส่วนลด” บนใบ **คือฐานก่อน VAT ไม่ใช่ยอดรวมลบส่วนลด** · 🔴 ยังไม่เขียนกฎตรวจยอด — เป็นของงาน 6.3 |
| D62 | 25 ก.ย. 2569 | “ไม่มีบนเอกสาร” ≠ “อ่านไม่ออก” | **ไม่มีบนเอกสาร กับ อ่านไม่ออก เป็นคนละสถานะ ต้องแยกเก็บ** · ที่มา: ใบส่งของชั่วคราวเขียนมือ (แบบ B) ไม่มี LOT และไม่มี EXP อยู่บนกระดาษเลย — ไม่ใช่ว่า AI อ่านพลาด · แบบ B คือของที่ร้านใช้จริงทุกวัน ไม่ใช่กรณียกเว้น |
| D63 | 25 ก.ย. 2569 | เครื่องหมายขีดซ้ำบรรทัด | เครื่องหมายซ้ำ เช่น `------ 180cc` หมายถึง **สินค้าเดิมบรรทัดบน แต่ขนาดนี้** · **สืบจากบรรทัดบน หรือแจ้งว่าไม่แน่ใจ ห้ามแต่งชื่อสินค้าขึ้นเอง** |
| D64 | 25 ก.ย. 2569 | วันที่กำกวม | ใบเดียวกันมีทั้ง พ.ศ. และ ค.ศ.: หัวใบ `04/09/2569` เป็น พ.ศ. · MFG/EXP ในบรรทัดสินค้าเป็น ค.ศ. 2 หลัก รูปแบบ วว.ดด.ปป · บางวันกำกวมจริง — `05.06.28` เป็นได้ทั้ง 5 มิ.ย. และ 6 พ.ค. · **อนุมานจากบรรทัดที่ไม่กำกวมในใบเดียวกัน ถ้าไม่มีเลย ให้คนตัดสิน ห้ามเดา** |
| D65 | 25 ก.ย. 2569 | ชุดทดสอบต้องมีใบเขียนมือ | **ชุดทดสอบการอ่านใบต้องมีใบเขียนมืออย่างน้อย 1 ใบเสมอ** · ห้ามวัดผลจากใบพิมพ์สวยอย่างเดียว · เหตุผล: ใบเขียนมือคือของที่ร้านใช้ทุกวัน ความแม่นยำที่วัดจากใบพิมพ์อย่างเดียวจะหลอกว่าระบบพร้อมใช้ |
| D43-e | 25 ก.ย. 2569 | ข้อความที่ assertion รายงาน ต้องค้นหาเจอในซอร์สจริง | 🔴 **สตริงที่ assertion พิมพ์ออกมาเวลา FAIL ต้องค้นหาเจอในไฟล์จริง ถ้ารายงานสตริงที่ไม่มีอยู่ในซอร์ส ถือว่าเป็นบั๊กของเครื่องมือตรวจ** ไม่ใช่เรื่องความสวยงาม · ที่มา: ตัวจับ `text-[...]` ฉบับแรกรายงานว่า `text-[13px]` ทั้งที่ซอร์สเขียน `sm:text-[13px]` — คนอ่านจะ Ctrl+F หาสตริงที่พิมพ์มาแล้วหาไม่เจอ สรุปว่าเป็น false positive แล้วปิดทิ้ง · ขยายความ: เลขบรรทัดก็ต้องตรงด้วย การลบคอมเมนต์ก่อนตรวจจึงต้องแทนด้วยช่องว่างและคงบรรทัดไว้ ไม่ใช่ลบบรรทัดทิ้ง |
| D43-d | 24 ก.ย. 2569 | assertion เรื่องการโต้ตอบ ต้องกดจริงแล้วดูผล | 🔴 **assertion ที่ตรวจการโต้ตอบ (กด · สลับ · เปิด/ปิด) ต้องเรียกตรรกะจริงแล้วดูผลลัพธ์ ห้ามตรวจแค่ว่า attribute มีอยู่** · 🔴 **“มี `aria-expanded`” ไม่ใช่หลักฐานว่า “สลับได้”** · ที่มา: assertion เรื่องการพับเมนูทั้งชุดตรวจแต่รูปร่างของโค้ด — มี `aria-expanded={open}` ไหม · `open` เขียนแบบไหน · ไฟล์เก็บสถานะพูดถึง `useSyncExternalStore` ไหม — ไม่มีสักตัวที่เปิดกลุ่ม แล้วปิด แล้วดูผล บั๊กกดแล้วไม่หดจึงผ่านไปถึงมือถือจริง · **และที่อันตรายกว่านั้น: มี assertion หนึ่งตัว “รับรอง” พฤติกรรมที่ผู้ใช้เรียกว่าบั๊กว่าถูกต้อง** (“ปุ่มของกลุ่มที่กำลังดูอยู่กดไม่ได้”) — assertion ที่ยืนยันของผิดอย่างมั่นใจ อันตรายกว่าไม่มี assertion เลย · วิธีทำ: ย้ายตรรกะออกจากคอมโพเนนต์เป็นฟังก์ชันล้วน (`lib/nav/folding.ts`) แล้วให้เทสต์รันมัน |
| D48 | 24 ก.ย. 2569 | ปรับขนาด/ระยะห่างทั้งระบบ ให้แก้ที่ตัวแปรกลาง | 🔴 **การปรับขนาดตัวอักษรหรือระยะห่างทั้งระบบ ต้องแก้ที่ตัวแปรกลาง ห้ามไล่แก้รายไฟล์** เพราะจะหลุดและไม่สม่ำเสมอ · Tailwind v4 แปลง `text-*` ทุกตัวเป็น `var(--text-*)` และทุก `gap/padding` เป็นทวีคูณของ `var(--spacing)` จึงนิยามตัวแปรใหม่ใน `@media (width < 40rem)` ที่ `globals.css` ครั้งเดียว ได้ผลทั้ง 17 หน้า โดยจอกว้างไม่ถูกแตะ · 🔴 **ข้อยกเว้นที่ต้องกวาดหา: ค่าตายตัวที่ไม่ได้อยู่บน scale** เช่น `text-[0.8rem]` `text-[11px]` `text-[10px]` — พวกนี้รอดจากการปรับตัวแปรเพราะไม่ได้อ่านค่าจาก scale ต้องย้ายขึ้นมาบน scale · กวาดครั้งแรก 24 ก.ย. 2569 เจอ 3 จุด: ปุ่มไซส์ `sm` (12.8px) · บรรทัดสถานะกลุ่มเมนู (**11px**) · ป้าย “กำลังดูอยู่” (**10px**) — สองตัวหลังอยู่ในเมนูซึ่งเป็นสิ่งที่ D47 ตั้งใจแก้พอดี แต่หลุดรอบแรกเพราะไม่ได้อยู่บน scale · ย้ายขึ้น `text-xs` ทั้งหมดแล้ว · 🔴 **ค่าตายตัวนอก scale ต้องมีเครื่องกันถาวร ไม่ใช่กวาดด้วยมือรอบเดียว** — การกวาดด้วยมือกันของที่มีอยู่ได้ แต่กันของที่จะเขียนพรุ่งนี้ไม่ได้ · assertion ถาวรอยู่ใน `source-rules.check.mjs` (ไฟล์ที่เดินอ่านซอร์สทุกไฟล์อยู่แล้ว): ห้ามมี `text-[...]` ใน `web/src` ทั้งหมด รวมแบบมี prefix (`sm:` `md:hover:`) ครอบ `.tsx` `.ts` `.css` · **รายการยกเว้น: ไม่มี** และถ้าจะมีต้องรายงานขออนุมัติก่อน ห้ามเพิ่มเงียบ ๆ · คอมเมนต์ถูกลบก่อนตรวจ (เคยมี check ที่จับคอมเมนต์ของตัวเองมาแล้ว) และข้อความที่ FAIL บอก **ไฟล์:บรรทัด พร้อมชื่อคลาสเต็มรวม prefix** |
| — | 24 ก.ย. 2569 | 14px คือพื้นของข้อความรองบนมือถือ | ข้อความรอง — badge สถานะ · ป้ายตัวกรอง · คำอธิบายใต้ช่องกรอก · ป้ายรองใต้ชื่อยา — อยู่ที่ **14px** บนมือถือ (ขึ้นจาก 12px) **เป็นการตัดสินใจ ไม่ใช่ของตกหล่น** · เนื้อหาหลักอยู่ที่ 16px ตาม D47-4 · เหตุผล: ถ้าทุกอย่างเท่ากันหมดจะไม่เหลือลำดับความสำคัญ และ D34 ต้องการ “อ่านออก” ไม่ใช่ “ทุกตัวเท่ากัน” · เส้นแบ่ง: อะไรที่เป็น **ข้อมูลของแถวนั้นจริง ๆ** = 16px (เช่นบรรทัด “ฟิลด์: ค่าเก่า → ค่าใหม่” ในหน้าตรวจสอบย้อนหลัง ซึ่งย้ายขึ้นแล้ว) · อะไรที่เป็น **ป้ายกำกับหรือเชิงอรรถ** = 14px · ข้อยกเว้นที่จงใจ: ลิงก์ที่อยู่ในประโยค (เช่น “ไปหน้าคลังยา”) คงความสูงตามบรรทัด เพราะกล่อง inline ไม่รับ `min-height` และการดันให้ใหญ่จะทำประโยคแตก |
| D46 | 24 ก.ย. 2569 | ข้อจำกัดทางกายภาพต้องคำนวณตั้งแต่ตอนออกแบบ | 🔴 **ขนาดจอ · ขนาดนิ้วมือ · ความเร็ว ต้องคำนวณและรายงานตั้งแต่ตอนเสนอแบบ ห้ามไปเจอตอนทำเสร็จแล้ว** · ที่มา: U-8 ต้องการ 1000 จุดให้จิ้มบนผัง — คำนวณก่อนเขียนโค้ดพบว่าจอ 375px หักขอบเหลือ 343px ห้องกว้าง 5377 มม. จึงได้ 1 px = 15.7 มม. · เป้านิ้วตามมาตรฐาน 44px = 690 × 690 มม. บนพื้นจริง (ใหญ่กว่าเสากลางร้านที่ 650 × 650 มม.) · พื้นที่ห้อง 31.2 ตร.ม. หารด้วย 0.476 ตร.ม. ต่อเป้า = **ใส่ได้ 65 จุด ไม่ใช่ 1000** · ถ้ารู้เรื่องนี้หลังทำเสร็จ แปลว่าต้องรื้อ UI ทั้งหน้า ไม่ใช่แก้ตัวเลข · สิ่งที่ต้องคำนวณเสมอก่อนเริ่ม: จำนวนของที่ต้องกดได้ต่อหนึ่งหน้าจอเล็กสุด · ขนาดที่เล็กที่สุดที่นิ้วกดโดน · จำนวนโหนด/แถวที่ต้องวาดพร้อมกัน · ตัวเลขความเร็วต้องมาจากการวัดจริง ห้ามเดาจากการอ่านโค้ดตัวเอง (D41) |
...
| D43 | 23 ก.ย. 2569 | เมนูพับได้ (กลับคำจาก U-3) | 🔴 **กลับคำจาก U-3** — คำวินิจฉัยเดิม “ห้ามพับ” ผิด เพราะผู้ใช้มองแล้วบอกว่ารก ซึ่งเป็นหลักฐานชั้นเดียวกับที่ยกเลิก Dumbbell (**D41**) · **กลุ่มพับเปิด-ปิดได้ หัวข้อกลุ่มเป็นปุ่มได้** · 🔴 **บล็อกบนสุด (หน้าแรก · ขายยา · คลังยา) ห้ามพับเด็ดขาด** เป็นงานทุกวัน ถ้าต้องกดเปิดก่อนขายยา = แย่ลงกว่าเดิม · 4 กลุ่มที่เหลือพับได้: หน้าที่ประจำวัน · ยาที่ต้องดูด่วน · รายงานและประวัติ · ตั้งค่า · **ครั้งแรกที่เข้า เปิดทุกกลุ่ม ไม่ใช่พับทุกกลุ่ม** (ต้องเห็นว่ามีอะไรบ้างก่อน) · **กลุ่มที่มีหน้าที่กำลังดูอยู่ ต้องเปิดเสมอ** แม้ผู้ใช้เคยพับไว้ · จำสถานะใน `localStorage` แยกตามผู้ใช้ตาม **D38** อ่านเขียนใน `try/catch` อ่านไม่ได้ = เปิดทุกกลุ่ม · หัวข้อที่พับได้ต้องเป็น `<button>` จริง มี `aria-expanded` และ **มีคำบอกสถานะเป็นข้อความ ไม่ใช่ลูกศรอย่างเดียว** (D34) · ใช้กติกาเดียวกันทั้ง PC และมือถือ ไม่แยกพฤติกรรม · 🔴 **Wheel Picker สำหรับเมนู ไม่อนุมัติ** — D39 ให้ wheel กับช่องกรอกตัวเลขที่มีทางสำรองคือพิมพ์เสมอ เมนูไม่มีทางสำรองแบบนั้น · wheel ซ่อนตัวเลือกส่วนใหญ่ไว้นอกจอ = ของหายจากสายตา ขัด D34 · ผู้สูงอายุหมุนพลาดง่าย และเมนูพลาดแล้วไปผิดหน้า · มือถือใช้รายการเต็มจอ + พับกลุ่มได้  · 🔴 **แก้ข้อความ 24 ก.ย. 2569:** เดิมเขียนว่า *“กลุ่มที่มีหน้าที่กำลังดูอยู่ ต้องเปิดเสมอ แม้ผู้ใช้เคยพับไว้”* → แก้เป็น **“กลุ่มที่มีหน้าที่กำลังดูอยู่ ให้เปิดเป็นค่าเริ่มต้น แต่ผู้ใช้ต้องพับได้เสมอ ห้าม `disabled` ปุ่มหัวข้อในทุกกรณี”** · **ที่มา:** ถ้อยคำ “เปิดเสมอ” ฉบับแรกถูกแปลเป็น `disabled={holdsCurrentPage}` ซึ่งเป็นการแปลที่ตรงตามตัวอักษร ผลคือผู้ใช้ที่อยู่หน้า “รับสินค้า” กดหัวข้อ “หน้าที่ประจำวัน” แล้วไม่มีอะไรเกิดขึ้น — ปุ่มตาย ซึ่งอ่านว่าพัง ไม่ใช่อ่านว่าเป็นกฎ (D34) · แก้จริงใน `01651d1` (D47-1) |
