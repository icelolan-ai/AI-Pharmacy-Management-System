...
| D21 | 19 ก.ย. 2026 | ข้อมูลร้าน (หัวใบเสร็จ) | ตาราง `store_profile` **ระเบียนเดียว** (Migration 005) — `GET /api/v1/store` ทุก Role ที่ล็อกอินอ่านได้ และตอบ `200` พร้อมค่า `null` เมื่อยังไม่เคยบันทึก (**ไม่ตอบ `404`**); `PATCH /api/v1/store` เฉพาะ **owner** (ครั้งแรกต้องมี `name`) และบันทึก `audit_logs` ด้วย `table_name = 'store_profile'` |
| D22 | 19 ก.ย. 2026 | *(รอข้อความจาก Chat A)* | ⏳ ยังไม่ได้รับเนื้อหา — จองหมายเลขไว้ก่อนเพื่อไม่ให้เลขซ้ำ |
| D23 | 19 ก.ย. 2026 | ปรับสต็อกพร้อมกันหลายคน | การปรับสต็อกบันทึก `quantity_before` ของ Lot ไว้ด้วย — ถ้ามีคนปรับ Lot เดียวกันพร้อมกัน ค่าที่บันทึกจะไม่ถูกเขียนทับ และตรวจย้อนหลังได้ว่าแต่ละครั้งเริ่มจากจำนวนเท่าไร |
